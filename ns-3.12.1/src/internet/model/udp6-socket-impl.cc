...
// Add attributes generic to all Udp6Sockets to base class Udp6Socket
TypeId
...
Udp6SocketImpl::DoSendTo (Ptr<Packet> p, const Address &address)
{
  ...
  if (!m_connected)
    ...
    {
      // connected UDPv6 socket must use default addresses
...
Udp6SocketImpl::DoSendTo (Ptr<Packet> p, Ipv6Address dest, uint16_t port)
{
  ...
  if (m_endPoint->GetLocalAddress () != Ipv6Address::GetAny ())
    {
...
// XXX maximum message size for UDPv6 broadcast is limited by MTU
